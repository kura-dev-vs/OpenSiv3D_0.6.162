...
public:
	...
	/// @brief HP バーを作成します。
	/// @param maxHP 最大 HP
	explicit constexpr UIBar(int32 maxHP) noexcept
		: m_maxHP{ maxHP }
		, m_currentHP{ maxHP }
	...
	constexpr UIBar(int32 maxHP, int32 currentHP) noexcept
		: m_maxHP{ maxHP }
	...
	void draw(const RectF& rect, const Style& style) const
	{
		...
		rectHP.draw(style.uiColor);
		rect.drawFrame(style.frameThickness, style.frameColor);
	}

	/// @brief 長方形の HP バーを描画します。追加
	/// @param rect 長方形
	void draw(const RectF& rect, ColorF uiColor) const
	{
		Style style;
		style.backgroundColor = ColorF{ 0.0, 0.6 };
		style.delayColor = ColorF{ 0.9, 0.8, 0.3 };
		style.uiColor = uiColor;
		style.frameColor = ColorF{ 0.1 };
		style.frameThickness = 1.5;
		draw(rect, style);
	}
